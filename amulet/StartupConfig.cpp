...
StartupConfig deserializeStartupConfig(char *str, uint8_t len)
{
	...
	config.pattern.params.mask_ = next_tok();
	config.pattern.params.filter_ = next_tok();
...
#define APPEND_INT_COMMA(X)                    \
	...
	APPEND_INT_COMMA(config.pattern.params.mask_)
	APPEND_INT_COMMA(config.pattern.params.filter_)
...
void testSerialization()
{
	...
	config.pattern.params.mask_ = 1;
	config.pattern.params.filter_ = 1;
