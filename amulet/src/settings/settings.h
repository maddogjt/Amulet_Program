...
#define EXTERNAL_LED_MAX_COUNT 144
...
typedef struct  __attribute__((packed))
{
    ...
    anim_config_t burnFollowPattern_{};
	...
	bool 		intLedEn_;
    bool		externalLedEn_;
    uint16_t 	externalLedCount_;
    bool 		externalLedExtend_;

	uint8_t brightness_[3];
	uint8_t extBrightness_[3];
    ...
    anim_config_t &getConfigModeAnim(amulet_mode_t mode)
    {
        ...
        if (mode == AMULET_MODE_BURN_FOLLOW)
        {
            return burnFollowPattern_;
        }
...
static_assert(sizeof(LocalSettings) == 78, "");
